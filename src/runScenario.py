...
SCENARIOS = ['FishFallsInBowl', 'HookTrapsFish', 
             'HookTrapsRing', 'GripperClapsStarfish']
...
class runScenario():
    ...
    def getJointStates(self):
        numJoints = p.getNumJoints(self.objectId)
        if numJoints == 0:  # rigid object with no joints
            return [], [], []
        joint_states = p.getJointStates(self.objectId, range(numJoints))
...
if __name__ == '__main__':
    ...
    for i in range(numMainIter):
        ...
        goal = sce.goalCoMPose + [0]*env.robot.articulate_num
        ...
        if args.search == 'BoundShrinkSearch':
            ...
            bestCostsSce.append(bestCost) # list(numMainIter*list(numInnerIter))
            startCostSce.append(startCost)
